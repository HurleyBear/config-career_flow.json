/* Career Direction Navigator — Wizard + Sidebar Compass
   - Loads config/career_flow.json
   - Diagnostic -> Recommendation -> Refinement -> Focus -> Plan -> Summaries
*/
...
const state = {
  phase: "loading", // intro | profile | diagnostic | recommendation | refinement | focus | plan | summary | share
  profile: {},
...
function addSignals(signalsObj) {
  ...
  for (const key of Object.keys(state.signals)) {
    if (typeof signalsObj[key] === "number") {
      state.signals[key] += signalsObj[key];
    }
  }
...
function addPathScore(deltaObj) {
  ...
  for (const [pathId, delta] of Object.entries(deltaObj)) {
    if (typeof delta === "number") {
      state.pathScores[pathId] = (state.pathScores[pathId] || 0) + delta;
    }
  }
}
...
function dominantSignal() {
  ...
  for (const id of core) {
    if (state.signals[id] > best.val) best = { id, val: state.signals[id] };
  }
  return best.id;
}
...
function computeConfidence() {
  ...
  for (const b of bands) {
    if (delta >= b.minDelta) { band = b.id; break; }
  }
...
function applyRoutingOverridesIfNeeded() {
  // called at diagnostic completion
  const domSig = dominantSignal();

  // rules from CONFIG.flow.routingRules
...
function listWhyBullets() {
  // pick top 2–3 dominant signals per summaryLogic rule
  const templates = CONFIG.summaryLogic.whyEvidence.templates;
  const core = ["depth","scope","breadth","recalibration"];
  const sorted = core
    .map(id => ({ id, val: state.signals[id] }))
...
function evidenceBulletsFromDecisionLog(max=4) {
  // Take a few answers and translate to evidence lines (simple version)
...
function computeIntentTranslation(primaryPath) {
  ...
  for (const rule of rules) {
    ...
    // match dotted keys in when (other than primaryPath)
    let ok = true;
...
function buildFocusStatement(primaryPath) {
  ...
  if (primaryPath === "levelUp") {
    const map = maps.levelUp?.byLevelUpType || {};
    descriptor = map[v.levelUpType] || "greater responsibility and impact";
  } else if (primaryPath === "thrive") {
    const map = maps.thrive?.byThriveFocus || {};
    // thriveFocus stored as one value; if multi, you can store array later
    descriptor = map[v.thriveFocus] || "my effectiveness and impact";
  } else if (primaryPath === "moveAcross") {
    const map = maps.moveAcross?.byAcrossPurpose || {};
    descriptor = map[v.acrossPurpose] || "broader perspective";
  } else if (primaryPath === "expandView") {
    const map = maps.expandView?.byExploreMode || {};
    descriptor = map[v.exploreMode] || "a short-term experience";
  } else if (primaryPath === "reset") {
    const map = maps.reset?.byResetDriver || {};
    descriptor = map[v.resetDriver] || "sustainability and clarity";
  }
...
function getDiagnosticQuestionQueue() {
  // In this v1, we simply use CONFIG.diagnostic.questions as the queue.
  // Tie-breakers are handled by inserting TB questions on the fly if needed.
  return CONFIG.diagnostic.questions;
}

function renderDiagnosticQuestion() {
  renderCompass();

  const queue = getDiagnosticQuestionQueue();
  ...
  const optionsHtml = q.options.map(opt => {
    ...
    return `
      <button class="option ${isSelected ? "selected" : ""}" data-opt="${escapeHtml(opt.id)}">
  ...
  function updateNextEnabled() {
    ...
    if (q.type === "multi") {
      $nextBtn.disabled = !(Array.isArray(ans) && ans.length > 0);
    } else {
  ...
  document.getElementById("backBtn").onclick = () => {
    ...
    // NOTE: For a true back button with scoring rollback, you'd re-run all answers from scratch.
    // In this prototype, we keep it simple: we re-run scoring from scratch on each render.
    recomputeFromDiagnosticAnswers();
    renderDiagnosticQuestion();
  };

  $nextBtn.onclick = () => {
    applyDiagnosticAnswerToState(q);
    state.diagnostic.index += 1;

    // Tie-break insertion after index 3 (0-based) if early confidence
    maybeInsertTieBreakers();
...
function recomputeFromDiagnosticAnswers() {
  // wipe then reapply all answers up to current index
  for (const k of Object.keys(state.signals)) state.signals[k] = 0;
  state.scales.ambiguity = null; state.scales.change = null; state.scales.readiness = null;
  initScores();
  state.decisionLog = [];
  ...
  for (let i = 0; i < state.diagnostic.index; i++) {
    const q = queue[i];
    if (!q) continue;
    const ans = state.diagnostic.answers[q.id];
    if (!ans) continue;

    // apply as if confirmed
    applyDiagnosticAnswerToState(q, true);
  }
  computeConfidence();
  renderCompass();
}

function applyDiagnosticAnswerToState(q, silent=false) {
  const ans = state.diagnostic.answers[q.id];
  if (!ans) return;

  // multi: apply each selected option
...
function maybeInsertTieBreakers() {
  const rules = CONFIG.diagnostic.tieBreakerRules;
  if (!rules?.enabled) return;

  const triggerIndex = rules.trigger?.afterQuestionIndex ?? -1;
  const bandList = rules.trigger?.confidenceBandIn || [];
  const inBand = bandList.includes(state.confidence.band);

  // Only once, after trigger point, if TB questions not already present
  const queue = CONFIG.diagnostic.questions;
  const alreadyHasTB = queue.some(q => (q.id || "").startsWith("TB"));
  if (alreadyHasTB) return;

  if (state.diagnostic.index === triggerIndex + 1 && inBand) {
    const tbIds = rules.questionIds || [];
    const tbQuestions = tbIds
      .map(id => CONFIG.diagnostic.questions.find(q => q.id === id))
      .filter(Boolean);

    // In our JSON, TB questions are already in the questions list, but the simplest pattern:
    // we *do nothing* here because they're already present.
    // If you later separate TB questions into their own list, insert them here.

    // For now, we’ll simply proceed—TBs are already part of the list in order.
    // (Kept this hook for v2.)
  }
}

function finishDiagnostic() {
  // compute top two from actual scores
  ...
  // apply routing rules (early signal -> expandView; dominant recalibration -> reset)
...
function renderRecommendation() {
  ...
  const why = listWhyBullets();
  const conf = state.confidence.label;
  ...
  $app.innerHTML = `
    ...
    <div class="grid2">
      <div class="pathPick">
        ...
        <p><strong>${escapeHtml(primary.label)}</strong><br/><span class="muted">Confidence: ${escapeHtml(conf)}</span></p>
        <div style="margin-top:10px" class="summaryBox">${escapeHtml(why)}</div>
      </div>
  ...
  $app.innerHTML = `
    ...
    <div class="grid2">
      <div class="pathPick">
        ...
        <p class="muted" style="margin-top:10px"><strong>Fits when:</strong><br/>You want progress that’s mainly about ${escapeHtml(compassHintFor(a.id))}.</p>
      ...
      <div class="pathPick">
        ...
        <p class="muted" style="margin-top:10px"><strong>Fits when:</strong><br/>You want progress that’s mainly about ${escapeHtml(compassHintFor(b.id))}.</p>
...
function compassHintFor(pathId) {
  const map = {
    thrive: "mastery, depth, and visible excellence in-role",
    levelUp: "bigger ownership, decisions, and responsibility",
    moveAcross: "breadth, learning, and a new lane",
    expandView: "exploration with structure and evidence",
    reset: "rebuilding sustainability, clarity, or skills"
  };
  return map[pathId] || "progress and clarity";
}
  ...
  const applyOption = (opt) => {
    ...
    if (opt.sets) {
      for (const [k,val] of Object.entries(opt.sets)) {
        // our sets keys are like "versionOfPath.peopleLeadership"
        // We apply them relative to v (strip "versionOfPath.")
...
function startPlan() {
  ...
  // Suggested experiments based on chosen path
  const defaults = pickDefaultExperimentsForPath(state.pathIntent.primaryPath);
  state.plan.experimentsSelected = defaults.slice();

  // Build focus statement from defaults (so it can reference experiment1)
...
function renderPlan() {
  ...
  const pathId = state.pathIntent.primaryPath;
  const suggested = pickDefaultExperimentsForPath(pathId);
  ...
  $app.innerHTML = `
    ...
    <div class="grid2">
      <div>
        ...
        <div class="smallNote">Tip: you can start with two. If you choose three, make one lightweight.</div>
      </div>

      <div>
        <div class="sectionTitle">Your draft focus statement</div>
  ...
  function refreshSelectedList() {
    const text = state.plan.experimentsSelected.map(e => `• ${e.label}`).join("\n") || "• (none yet)";
    $selectedList.textContent = text;
  }
  ...
  $expOptions.querySelectorAll("button[data-exp]").forEach(btn => {
    btn.onclick = () => {
      ...
      if (isSelected(id)) {
        ...
        if (state.plan.experimentsSelected.length < max) {
          state.plan.experimentsSelected = [...state.plan.experimentsSelected, e];
        }
      }

      // Update focus statement “experiment1” reference if needed
      state.focusStatement = document.getElementById("focusText").value;
      refreshSelectedList();
      renderPlan(); // easiest re-render
    };
  });

  document.getElementById("backBtn").onclick = () => {
    // back to refinement end
    startRefinement(state.pathIntent.primaryPath);
    // jump to last refinement screen
    const set = CONFIG.refinement.questionSets[state.pathIntent.primaryPath] || [];
...
${CONFIG.templates.employeeSummary.sections.find(s=>s.id==="leaning").format
  .replace("{primaryPathLabel}", primary.label)
  .replace("{secondaryPathLabel}", secondary.label)
  .replace("{confidenceLabel}", state.confidence.label)
}
...
${CONFIG.templates.leaderSummary.sections.find(s=>s.id==="translation").format
  .replace("{primaryPathLabel}", primary.label)
  .replace("{intentTranslation}", translation.intentTranslation || "")
}

${CONFIG.templates.leaderSummary.sections.find(s=>s.id==="evidence").label}
${evidenceBullets}

${CONFIG.templates.leaderSummary.sections.find(s=>s.id==="coaching").label}
${coachingBullets}

${CONFIG.templates.leaderSummary.sections.find(s=>s.id==="experiments").label}
${experimentBullets}

${CONFIG.templates.leaderSummary.sections.find(s=>s.id==="watchouts").label}
${watchoutBullets}
`;
...
async function boot() {
  state.phase = "loading";
  $app.innerHTML = `<div class="screenTitle">Loading</div><h2 class="prompt">Loading configuration…</h2><p class="subcopy muted">If you’re running locally, you may need to use a local server (not “file://”).</p>`;
...
boot().catch(err => {
  console.error(err);
  $app.innerHTML = `<div class="screenTitle">Error</div><h2 class="prompt">Could not load config.</h2><p class="subcopy">Make sure <code>${CONFIG_URL}</code> exists and you’re running via a local server or GitHub Pages.</p>`;
});
